...
public class SumNormalizor extends Normalizer {
	...
	public void normalize(RankList rl) {
		if (rl.size() == 0) {
			System.out
					.println("Error in SumNormalizor::normalize(): The input ranked list is empty");
			System.exit(1);
		}
		...
		for (int i = 0; i < rl.size(); i++) {
			...
			for (int j = 1; j <= nFeature; j++) {
				if (norm[j - 1] > 0) {
					if (dp.getFeatureValue(j) > 0) {
						dp.setFeatureValue(j,
								(float) (dp.getFeatureValue(j) / norm[j - 1]));
					}
				}
			}
		}
	...
	public void normalize(RankList rl, int[] fids) {
		...
		// remove duplicate features from the input @fids ==> avoid normalizing
		// the same features multiple times
		...
		for (int i = 0; i < rl.size(); i++) {
			...
			for (int j = 0; j < fids.length; j++)
				if (norm[j] > 0) {
					if (dp.getFeatureValue(fids[j]) > 0) {
						dp.setFeatureValue(fids[j],
								(float) (dp.getFeatureValue(fids[j]) / norm[j]));
					}
				}
		}
	}
