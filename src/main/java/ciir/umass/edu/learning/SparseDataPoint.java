...
public class SparseDataPoint extends DataPoint {
	...
	public SparseDataPoint(String text, Set<Integer> listFeatures) {
		super(text, listFeatures);
	}

	public SparseDataPoint(SparseDataPoint dp) {
		label = dp.label;
		this.user_id = dp.user_id;
	...
	private int locate(int fid) {
		if (searchPattern == accessPattern.SEQUENTIAL) {
			...
			System.err
					.println("Invalid search pattern specified for sparse data points.");
	...
	public float getFeatureValue(int fid) {
		...
		if (fid <= 0 || fid > getFeatureCount()) {
			return 0;
			// System.out.println("Error in SparseDataPoint::getFeatureValue(): requesting invalid feature, fid="
			// + fid);
			// System.exit(1);
	...
	public void setFeatureValue(int fid, float fval) {
		if (fid <= 0 || fid > getFeatureCount()) {
			System.out
					.println("Error in SparseDataPoint::setFeatureValue(): feature (id="
							+ fid + ") out of range.");
			System.exit(1);
		}
		int pos = locate(fid);
	...
	public float[] getFeatureVector() {
		System.out.println("call to get feature vector method");
		if (knownFeatures > 0) {
			float[] dfVals =  new float[fIds[knownFeatures -1]]; //knownFeatures-1
