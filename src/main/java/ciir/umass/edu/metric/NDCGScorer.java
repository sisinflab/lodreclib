...
public class NDCGScorer extends DCGScorer {
	...
	public void loadExternalRelevanceJudgment(String qrelFile) {
		// Queries with external relevance judgment will have their cached ideal
		// gain value overridden
		try {
			String content = "";
	...
	public double score(RankList rl) {
		...
		if (ideal <= 0.0)// I mean precisely "="
			return 0.0;
//		double dcg = getDCG(rel, size);
//
//		double ndcg = dcg / ideal;
//		System.out.println("--- ndcg: " + ndcg + ". dcg:" + dcg + ". idcg:"
//				+ ideal);
	...
	public double[][] swapChange(RankList rl) {
		...
		for (int i = 0; i < size; i++)
			for (int j = i + 1; j < rl.size(); j++)
				if (ideal > 0)
					changes[j][i] = changes[i][j] = (discount(i) - discount(j))
							* (gain(rel[i]) - gain(rel[j])) / ideal;
