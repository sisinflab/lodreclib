...
import it.poliba.sisinflab.LODRecommender.tree.NNode;
 ...
 */
public class TreeQueryExecutor implements Runnable {
	...
	private static Logger logger = LogManager.getLogger(TreeQueryExecutor.class.getName());
	
	
	/**
	 * Constuctor
	 */
	public TreeQueryExecutor(String uri, int uri_id,  
			NTree props, TObjectIntHashMap<String> props_index, String graphURI, String endpoint, 
	 ...
	 */
	public TreeQueryExecutor(String uri, int uri_id, NTree props, 
			...
			for (NNode children : node.getChilds()) {
				...
				if(result.size()>0){
					...
					for(String uri_res : result.keySet()){
						...
						int id_res = extractKey(uri_res);
							
						if(list_props.length()>0){
							itemTree.addBranches(list_props + "-" + p_index + "#" + id_res, id_res);
							exec(children, list_props + "-" + p_index + "#" + id_res, uri_res);
						}
						else{
							itemTree.addBranches(p_index + "#" + id_res, id_res);
							exec(children, p_index + "#" + id_res, uri_res);
						}
					}
				}
	        }
