...
public class TextFileUtils {
	...
	public static void writeTIntMapArrayString(String file,
			TIntObjectHashMap<ArrayList<String>> map) {
	...
	public static void loadTIntMapArrayString(String file,
			TIntObjectHashMap<ArrayList<String>> map) {
	...
	public static void loadTIntMapTIntHashSet(String file,
			TIntObjectHashMap<TIntHashSet> map) {
	...
	public static void writeTIntMapTIntHashSet(String file,
			TIntObjectHashMap<TIntHashSet> map) {
	...
	public static void writeData(String file,
			TIntObjectHashMap<TIntObjectHashMap<TIntIntHashMap>> path) {
		...
		try {
			...
			TIntObjectIterator<TIntObjectHashMap<TIntIntHashMap>> it = path
					.iterator();
			while (it.hasNext()) {
				it.advance();
				int a = it.key();
	...
	public static void loadInputURIs(String file,
		...
		try {
			...
			int index = 1;
			if (append) {
				if (new File("input_uri_id").exists()) {
	...
	public static void write(String file, TObjectIntHashMap<String> uri_id,
			boolean append) {
	...
	public static void loadInputUsersRatings(String file,
			TIntObjectHashMap<TIntFloatHashMap> user_rating,
			TFloatHashSet labels) {
	...
	public static void loadFileIndex(String file,
			THashMap<String, String> items_pair_value) {
	...
	public static void loadFileIndex(String file,
			TIntObjectHashMap<String> id_value) {
	...
	public static void computeIndex(String file,
			TObjectIntHashMap<String> value_id, HashSet<String> labels) {
	...
	public static void loadInputMetadataID(String metadata_file_index,
			String input_uri, TIntIntHashMap input_metadata_id) {
			...
			while ((line = br.readLine()) != null) {
				...
				if (vals.length == 2) {
					if (metadata_index.containsKey(vals[1]))
						input_metadata_id.put(Integer.parseInt(vals[0]),
								metadata_index.get(vals[1]));
				}
			}
	...
	public static void loadRelatedURIs(String file,
			TIntObjectHashMap<TIntArrayList> related_uri,
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				for (int i = 1; i < vals.length; i++) {
					if (!vals[i].contentEquals("null")
							&& uri_id.containsKey(vals[i]))
